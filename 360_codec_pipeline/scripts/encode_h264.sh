...
if [[ "$encoder" == nvenc ]]; then
 encoder_available h264_nvenc || die 'h264_nvenc is unavailable; run inspect_hardware.sh or use --encoder cpu'
 args+=(-c:v h264_nvenc -preset "$preset" -rc "$rc")
 [[ -n "$qp" ]] && args+=(-qp "$qp"); [[ -n "$bitrate" ]] && args+=(-b:v "$bitrate")
else
 encoder_available libx264 || die 'libx264 is unavailable in this FFmpeg build'
 args+=(-c:v libx264 -preset "$preset")
 [[ -n "$qp" ]] && args+=(-qp "$qp"); [[ -n "$bitrate" ]] && args+=(-b:v "$bitrate")
fi
args+=(-c:a copy "$output"); log_info "Encoding H.264 with $encoder"; ffmpeg "${args[@]}"
